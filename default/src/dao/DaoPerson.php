...
class DaoPerson {
    ...
    public function add(Person $pers) {
        ...
        try {
            ...
            $query = Connect::getInstance()->prepare('INSERT INTO person (name,email,password) VALUES (:name, :email, :password)');
            /**
    ...
    public function update(Person $pers) {
        ...
        try {
            ...
            $query = Connect::getInstance()->prepare('UPDATE person SET name = :name, email = :email, password = :password WHERE id = :id');
            //on bind les value des placeholders
            $query->bindValue(':name',$pers->getName(),\PDO::PARAM_STR);
            
            $query->bindValue(':email',$pers->getEmail(),\PDO::PARAM_STR);
            $query->bindValue(':password',$pers->getPassword(),
            \PDO::PARAM_STR);
